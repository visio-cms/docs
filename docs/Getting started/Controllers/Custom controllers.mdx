...
}: Pick<RenderBlockControllerProps, 'propIndex' | 'defaultValue' | 'handlePropValueChange'> & { prop: DatePickerT }) {
  return (
...
In this integration, we utilize specific properties such as `prop`, `propIndex`, `defaultValue`, and `handlePropValueChange` from `RenderBlockControllerProps` provided by `visio-cms`. These properties are essential for visio to correctly pass the value of the selected date to the respective block.
